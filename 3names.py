...
if file1 and file2:
    ...
    with c1:
        ...
        # التعديل هنا: اختيار عدة أعمدة لجلبها
        fetch_cols_1 = st.multiselect("اختر الأعمدة المراد جلبها من ملف بصرة عند المطابقة (مثل IBAN وغيره)", [c for c in df1.columns if c not in [name_col_1, school_col_1]])
        ...
        with st.spinner("جاري المطابقة..."):
            ...
            for _, db_row in df2.iterrows():
                ...
                if len(candidates) == 0:
                    ...
                    for col in fetch_cols_1: entry[col] = "" # أعمدة فارغة
                ...
                for col in fetch_cols_1:
                    entry[col] = best_row[col] if school_ok else ""
